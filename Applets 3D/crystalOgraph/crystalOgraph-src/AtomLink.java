...
public class AtomLink extends BranchGroup implements ColorConstants {
	...
	public AtomLink(Cell cell, Atom a1, Atom a2, float radius, Color3f color) {
		...
		this.cyl = createCylinder(cell.coord(a1.pos), cell.coord(a2.pos), radius, app, 12);
		this.addChild(this.cyl);
		Shape3D shape = ((Cylinder) ((Group) this.cyl.getChild(0)).getChild(0)).getShape(Cylinder.BODY);
	...
	public void changeColor(Color3f c) {
		...
		((Cylinder) ((Group) this.cyl.getChild(0)).getChild(0)).setAppearance(app);
	}

	public void changeRadius(float r) {
		this.radius = r;

		this.removeChild(this.cyl);
		Appearance app = new Appearance();
		app.setMaterial(new Material(this.color, black, this.color, white, 120.0f));
		this.cyl = createCylinder(this.cell.coord(this.a1.pos), this.cell.coord(this.a2.pos), r, app, 12);
		Shape3D shape = ((Cylinder) ((Group) this.cyl.getChild(0)).getChild(0)).getShape(Cylinder.BODY);
		shape.setUserData(this);
		try {
			shape.getGeometry().setCapability(Geometry.ALLOW_INTERSECT);
	...
	public void select() {
		...
		this.sel = createCylinder(this.cell.coord(this.a1.pos), this.cell.coord(this.a2.pos), this.radius * 1.5f, app,
				12);
		this.sel.setCapability(BranchGroup.ALLOW_DETACH);
		this.addChild(this.sel);
		this.selected = true;
	}

	public void unSelect() {
		this.removeChild(this.sel);
		this.selected = false;
		this.sel = null;
	}
	...
	public static BranchGroup createArrow(Point3d b, Point3d a, double radius, Appearance cylApp, int precision) {
		...
		Cone cne = new Cone((float) radius * 3, (float) radius * 10, Primitive.GENERATE_NORMALS, precision, 1, cylApp);

...
public class DataBaseCreator {
	...
	public final static String[][] doubleChoicesParam = {
			{"&gua=b", "&gua=c"},
			{"&gor=1", "&gor=2"},
	...
	public static void main(String[] args) throws MalformedURLException, IOException, InterruptedException {
		...
		if (1==1) throw new RuntimeException("Really !?");
		...
		for (int n=n0; n<=n1; n++) {
			...
			for (int c=0; c<=(CellSymetries.hasChoice(n)?1:0); c++) {
				...
				out.println(" public static float[][] pos"+n+(c==1?"alt":"")+"(float x, float y, float z) {return new float[][] {");
				...
				try {
					URL url = new URL("http://www.cryst.ehu.es/cgi-bin/cryst/programs/nph-getgen?list=show&what=gp&gnum="+n+param);
				...
				while (true) {						
					...
					if (i2!=-1) {
						...
						for (int i=0; i<eq.length(); i++) {
							char ch = eq.charAt(i);
							buf.append(ch);
							if (Character.isDigit(ch)) buf.append('f');
						}
						eq = buf.toString();				
						out.println("  {"+eq+"},");
					}
				...
				if (!endOk) throw new RuntimeException("End pattern not found");				
				if (!startOk) throw new RuntimeException("Start pattern not found");				
				...
				if (n%20==0) System.out.println(n);
		...
		for (int n=n0; n<=n1; n++)
			out.println("    CellGen.class.getMethod(\"pos"+n+"\", new Class[]{float.class, float.class, float.class}),");

		for (int n=n0; n<=n1; n++)
			if (CellSymetries.hasChoice(n))
				out.println("    CellGen.class.getMethod(\"pos"+n+"alt\", new Class[]{float.class, float.class, float.class}),");
		...
		out.println("  try {");
		out.println("   return (float[][]) methods[CellSymetries.getIndex(n, c)].invoke(null, new Object[]{new Float(x), new Float(y), new Float(z)});");

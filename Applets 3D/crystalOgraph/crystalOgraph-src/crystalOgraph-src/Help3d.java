...
public class Help3d extends BranchGroup implements ColorConstants, ActionListener {
	...
	public Help3d(crystalOgraph applet, RightPanel rightPanel) {
		...
		URL u = getClass().getResource("/help.html");
		try {
			txt.setPage(u);
		...
		txt.addMouseMotionListener(mmListener);
		txt.addMouseListener(mListener);
	}
	...
	MouseMotionListener mmListener = new MouseMotionAdapter() {
	  ...
	  public void mouseMoved(MouseEvent me)   {  	
	  	...
	  	if (pos >= 0)  	{  		
	  		...
	  		*/
	  		if (as.isDefined(javax.swing.text.html.HTML.Tag.A)) jt.setCursor(handCursor);  		
	  		else jt.setCursor(defCursor);  		
	  	}
	  }
	};
	  		...
	  		if (as.isDefined(javax.swing.text.html.HTML.Tag.A)) {
	  			...
	  			if (a!=null) {
	  				if (a.charAt(0)=='_') {
	  					...
	  					Component c = findComponent(applet.mainPane, a);
	  					if (c!=null) blink(c);
	  				}
	  				else try {                
  	  				txt.setPage(a);
	...
	public static Component findComponent(Container root, String name) {
		for (int i=0; i<root.getComponentCount(); i++) {
			...
			if (c instanceof JPanel && ((JPanel)c).getBorder()!=null && (((JPanel)c).getBorder() instanceof TitledBorder)) {
				String s = ((TitledBorder)((JPanel)c).getBorder()).getTitle(); 
				if (s.equals(name)) return c;
				else {
					int p = name.indexOf(s+"/");
					if (p!=-1) return findComponent((Container)c, name.substring(p+s.length()+1));
				}
			}
...
//this.addChild(Atom.createLegend("Help", new Point3d(0,0,0), new Point3d(0,0,0), 2, app));

...
public class SpaceGroup {
	...
	public SpaceGroup(SgType sg, Lattice cell) {
		this.sg=sg;
		this.cell=cell;
		system=SgSystem.getSystem(sg.dim, sg.systemIndex, sg.systemVariantIndex);
	}
	
	public Point2D.Double[] getSymPos(Point2D.Double p) { // not uniques !!
		Point2D.Double[] r = new Point2D.Double[sg.symPos.length];
		for (int i=0; i<r.length; i++) {
			r[i] = modCell(sg.symPos[i].mul(p));
		}
		return r;
	}
	
	public Point3d[] getSymPos(Point3d p) {	// unique positions (may be slow..)
		Point3d[] r = new Point3d[sg.symPos.length];
		for (int i=0; i<r.length; i++) {
			r[i] = modCell(sg.symPos[i].mul(p));
	...
	public Vector3d[] getSymPos(Vector3d p) {	// unique positions (may be slow..)
		Vector3d[] r = new Vector3d[sg.symPos.length];
		for (int i=0; i<r.length; i++) {
			r[i] = modCell(sg.symPos[i].mul(p));
			Point3d pi = new Point3d(r[i]);
			cell.transform(pi);
	...
	public static Vector3d modCell(Vector3d p) {
		return new Vector3d(p.x<0?(1-((-p.x)%1))%1:p.x%1, p.y<0?(1-((-p.y)%1))%1:p.y%1, p.z<0?(1-((-p.z)%1))%1:p.z%1);	
	}
	public static Point2D.Double modCell(Point2D.Double p) {
		return new Point2D.Double(p.x<0?(1-((-p.x)%1))%1:p.x%1, p.y<0?(1-((-p.y)%1))%1:p.y%1);	
	}

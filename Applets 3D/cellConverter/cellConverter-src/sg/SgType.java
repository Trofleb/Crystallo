...
public class SgType implements Serializable {
	...
	public SgType(int dim, int no, int dotNo, int variant, String name, String fullName, String[] otherNames,
			String suffix, String variantDesc, int systemIndex, int systemVariantIndex, MatVect[] symPos) {
		this.dim = dim;
		this.no = no;
		this.dotNo = dotNo;
	...
	public static void staticInit() {
		...
		try {
			BufferedReader in = new BufferedReader(
					new InputStreamReader(SgType.class.getResourceAsStream("/spacegroups.txt")));
			...
			while (true) {
				...
				if (sysi == -1 || sysv == -1)
					throw new RuntimeException("System not found " + ss[7] + " " + ss[8]);
				...
				HashMap m1, m2;
				if (mm[i].containsKey("" + no))
					m1 = (HashMap) mm[i].get("" + no);
				else
					mm[i].put("" + no, m1 = new HashMap());

				if (m1.containsKey("" + dotNo))
					m2 = (HashMap) m1.get("" + dotNo);
				else
					m1.put("" + dotNo, m2 = new HashMap());
			...
			for (int i = 0; i < SgSystem.dimensions.length; i++) { // dimension
				...
				for (int j = 0; j < sgs[i].length; j++) { // sg no
					...
					for (int k = 0; k < sgs[i][j].length; k++) { // dot no
						...
						for (int l = 0; l < sgs[i][j][k].length; l++)
	...
	public static SgType getSg(String name) {
		...
		for (int j = 0; j < sgs[d].length; j++) {
			if (sgs[d][j][0].length > v) {
				if (sgs[d][j][0][v].name.equals(name))
					return sgs[d][j][0][v];
				if (sgs[d][j][0][v].fullName.equals(name))
					return sgs[d][j][0][v];
			}
			for (int i = 0; i < sgs[d][j][0].length; i++)
				if (sgs[d][j][0][i].fullName.equals(name))
					return sgs[d][j][0][i];
		}
		return null;
	}
	...
	public String toString() {
		// return dim+"D "+no+(dim<=3?"":"."+dotNo)+" ("+variant+") "+name+"
		// "+(fullName.equals(name)?"":fullName)+" "+suffix;

...
public class CifFile {
	...
	public CifFile(File f) {
		try {
			this.parser = new CifFileParser(new FileReader(f));
	...
	public CifFile(Reader in) {
		this.parser = new CifFileParser(in);
	}

	public CifFile(String[] file) {
		this.parser = new CifFileParser(file);
	}

	public String getFormula() {
		String s = this.parser.getStringField("_chemical_formula_structural", "");
		if (s.trim().length() == 0)
			s = this.parser.getStringField("_chemical_formula_sum", "");
		return s;
	}

	public int getUnitZ() {
		return this.parser.getIntField("_cell_formula_units_Z", 1);
	}

	public String[] getData() {
		Vector v = new Vector(100, 100);
		v.addAll(this.parser.data);
		for (int i = 0; i < this.extraAtoms.size(); i++) {
			int l = ((Integer) this.extraAtomsLine.get(i)).intValue();
			v.add(l + 1, this.extraAtoms.get(i));
	...
	public SpaceGroup getSg() {
		String sgSymbol = this.parser.getStringField("_symmetry_space_group_name_H-M", "");
		int sgNo = this.parser.getIntField("_symmetry_Int_Tables_number", 0);

		// System.out.println("no:"+sgNo+" symbol:'"+sgSymbol+"'");

		double a = this.parser.getDoubleField("_cell_length_a", 5);
		double b = this.parser.getDoubleField("_cell_length_b", 5);
		double c = this.parser.getDoubleField("_cell_length_c", 5);
		double alpha = this.parser.getDoubleField("_cell_angle_alpha", 90);
		double beta = this.parser.getDoubleField("_cell_angle_beta", 90);
		double gamma = this.parser.getDoubleField("_cell_angle_gamma", 90);

		Lattice lattice = new Lattice(a, b, c, alpha, beta, gamma);
		if (sgSymbol.length() == 0) {
			if (sgNo == 0) {
				CifFileParser.Loop loop = this.parser.getLoopContainingHeader("_symmetry_equiv_pos_as_xyz");
				int indexXyz = loop == null ? -1 : loop.header.indexOf("_symmetry_equiv_pos_as_xyz");
				if (indexXyz == -1) {
					// throw new RuntimeException("No space group information
					// found");
				...
				for (int i = 0; i < loop.lines.size(); i++)
					// double x1 =
					// CifFileParser.parseCoord((String)lin.get(indexXin));
				...
				// throw new RuntimeException("Invalid spacegoup : "+sgNo+"
				// '"+sgSymbol+"'");
			}
	...
	public void updateLattice(Lattice cell, int uzFactor) {
		if (this.parser.isFieldPresent("_cell_formula_units_Z"))
			this.parser.setIntField("_cell_formula_units_Z", uzFactor);
		if (this.parser.isFieldPresent("_cell_volume"))
			this.parser.setDoubleField("_cell_volume", cell.volume());
		this.parser.setDoubleField("_cell_length_a", cell.a);
		this.parser.setDoubleField("_cell_length_b", cell.b);
		this.parser.setDoubleField("_cell_length_c", cell.c);
		this.parser.setDoubleField("_cell_angle_alpha", cell.alpha);
		this.parser.setDoubleField("_cell_angle_beta", cell.beta);
		this.parser.setDoubleField("_cell_angle_gamma", cell.gamma);
	}

	public void setSgP1() {
		if (this.parser.isFieldPresent("_symmetry_Int_Tables_number"))
			this.parser.setIntField("_symmetry_Int_Tables_number", 1);
		if (this.parser.isFieldPresent("_symmetry_space_group_name_H-M"))
			this.parser.setStringField("_symmetry_space_group_name_H-M", "'P 1'");
		CifFileParser.Loop loopEq = this.parser.getLoopContainingHeader("_symmetry_equiv_pos_as_xyz");
		if (loopEq != null) {
			loopEq.header.indexOf("_symmetry_equiv_pos_as_xyz");
			...
			for (int i = 0; i < loopEq.lines.size(); i++) {
				...
				if (i == 0) {
					if (indexId == -1)
						this.parser.data.set(dl, "  'x, y, z'");
					else
						this.parser.data.set(dl, "  1      'x, y, z'");
				} else
					this.parser.data.set(dl, "");
			}
		}
	}

	public void updateAtoms(AtomSite[] atoms) {
		CifFileParser.Loop loopPos = this.parser.getLoopContainingHeader("_atom_site_fract_x");
		...
		this.extraAtoms.clear();
		this.extraAtomsLine.clear();

		for (int i = 0; i < atoms.length; i++) {
			StringBuffer sb = new StringBuffer();
			for (int j = 0; j < loopPos.header.size(); j++)
				if (j == indexLabel)
					sb.append(atoms[i].label + "    ");
				else if (j == indexSymb)
					sb.append(atoms[i].atom + "    ");
				else if (j == indexX)
					sb.append(round(atoms[i].x) + "    ");
				else if (j == indexY)
					sb.append(round(atoms[i].y) + "    ");
				else if (j == indexZ)
					sb.append(round(atoms[i].z) + "    ");
				else if (j == indexOcc)
					sb.append(round(atoms[i].occupancy) + "    ");
				else if (j == indexMult)
					sb.append(atoms[i].multiplicity + "    ");
				else if (j == indexWyck)
					sb.append(atoms[i].wyckoff + "    ");
				else if (j == indexIsoB)
					sb.append(round(atoms[i].isoB) + "    ");
			if (i < loopPos.lines.size()) {
				loopPos.lines.get(i);
				int dl = ((Integer) loopPos.linesToDataLine.get(i)).intValue();
				this.parser.data.set(dl, sb.toString());
				last = Math.max(last, dl);
			} else {
				this.extraAtoms.add(sb.toString());
				this.extraAtomsLine.add(new Integer(last++));
			}
		}
	}

	// public void transformAtoms(CifFile cifIn, Matrix3d M, Vector3d v, Vector
	// translations) {
	// // (x',y',z') = M(x,y,z) + v
	// CifFileParser.Loop loopIn =
	// cifIn.parser.getLoopContainingHeader("_atom_site_fract_x");
	...
	//
	// CifFileParser.Loop loopOut =
	// parser.getLoopContainingHeader("_atom_site_fract_x");
	...
	//
	// for (int k=0; k<=translations.size(); k++) { // first one is 'no
	// translation'
	// StringBuffer sb = new StringBuffer();
	...
	// else if (!first&&j==indexLout)
	// sb.append(label+(k==0?"":("t"+k))+(l2==0?"":("s"+l2))+" ");
	...
	public AtomSite[] getAtoms() {
		...
		for (int i = 0; i < this.parser.loops.size(); i++) {
			CifFileParser.Loop loop = (CifFileParser.Loop) this.parser.loops.get(i);
		...
		for (int i = 0; i < loopCoord.lines.size(); i++) {
			...
			try {
				...
				String label = "";
				if (indexLabel != -1)
					label = (String) l.get(indexLabel);

				String atom = "O";
				if (indexSymb != -1)
					atom = (String) l.get(indexSymb);

				String symbol = "O";
				if (indexSymb != -1)
					symbol = this.getSymbol(atom);
				else if (indexLabel != -1)
					symbol = this.getSymbol(label);

				double occ = 1;
				if (indexOcc != -1)
					occ = CifFileParser.parseCoord((String) l.get(indexOcc));

				double oxid = 0;
				if (oxidations == null) {
					if (indexSymb != -1)
						oxid = this.getOxidation((String) l.get(indexLabel));
				} else {
					String s = (String) oxidations.get(atom);
					if (s != null)
						oxid = CifFileParser.parseCoord(s);
				}

				double isoB = 0;
				if (indexIsoB != -1)
					isoB = CifFileParser.parseCoord((String) l.get(indexIsoB));

				int mult = 1;
				if (indexMult != -1)
					mult = Integer.parseInt((String) l.get(indexMult));

				char wyck = 'a';
				if (indexWyck != -1)
					wyck = ((String) l.get(indexWyck)).charAt(0);

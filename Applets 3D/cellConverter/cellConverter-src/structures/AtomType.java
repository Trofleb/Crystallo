...
/**
 * AtomData class (accomodates for data related to atom types) A zero value of
 * any of its fields means this field is not present for the specific atom type.
 * Data are take from
 * <a href="http://www2.shef.ac.uk/chemistry/web-elements">http://www2.shef.ac.
 * uk/chemistry/web-elements</a>.
 */
class AtomData {
	/**
	 * The atomic number.
	...
	/**
	 * Constructor defining all data fields. 0.0 is used for unknown data
	 * fields.
	 */
	public AtomData(int nmbr, double wght, String smbl, String nm, double atRds, double covRds, double vdWrds,
			double metRds, int clr_idx) {
		this.number = nmbr;
		this.weight = wght;
		this.symbol = smbl;
		this.name = nm;
		this.atRadius = atRds;
		this.covRadius = covRds;
		this.vdWradius = vdWrds;
		this.metRadius = metRds;
		this.color_idx = clr_idx;
	}
}
...
/**
 * AtomType class (representation for atom types) Uses a static array with data
 * for each element.
 *
 * @see AtomData
 */
public class AtomType {
	...
	/**
	 * Return the atomic radius. 0.0 is returned in the case it is not
	 * specified.
	 */
	...
	/**
	 * Return the covalent radius. 0.0 is returned in the case it is not
	 * specified.
	 */
	...
	/**
	 * Return the radius metallic(12). 0.0 is returned in the case it is not
	 * specified.
	 */
	...
	/**
	 *
	 * Return the van der Waals radius. 0.0 is returned in the case it is not
	 * specified.
	 */
	...
	public boolean equals(Object o) {
		try {
			...
			return this.impl.number == at.impl.number;
		} catch (Exception e) {
			return false;
		}
	}
	...
	public synchronized String toString() {
		return new String("ATOM(" + this.impl.name + ')');
	}

	/**
	 * A static array containing the actual atom data. Data are obtained from
	 * http://www2.shef.ac.uk/chemistry/web-elements
	 */
	protected static final AtomData data[] = { new AtomData(0, 0.0, "NONE", "NONEXISTENT", 0.0, 0.0, 0.0, 0.0, 12),
	...
	/**
	 * A color index table. the RGB values in this table are referred to by the

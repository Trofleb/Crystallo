...
public class Transformer {
	...
	public Transformer(Matrix3d P, Matrix3d Q, Vector3d q) {
		...
		this.Q = Q;
		this.q = q;
		this.sgP1 = SgType.getSg(1);
		this.sgOut = new SpaceGroup(this.sgP1, this.latticeOut);
	}
	...
	public void setTranform(Matrix3d P, Matrix3d Q, Vector3d q) {
		...
		this.Q = Q;
		this.q = q;
		this.calculateLatticeOut();
		this.calculateAtomsOut();
	}
	...
	public AtomSite[] getTransformedAtoms() {
		return this.atomsOut;
	}

	public SpaceGroup getTransformedSg() {
		this.sgOut.cell = this.latticeOut;
		return this.sgOut;
	}

	private void calculateLatticeOut() {
		Matrix3d m = new Matrix3d();
		m.setColumn(0, this.sgIn.cell.x);
		m.setColumn(1, this.sgIn.cell.y);
		m.setColumn(2, this.sgIn.cell.z);
		m.mul(this.P);
		...
		m.getColumn(1, y);
		m.getColumn(2, z);
		this.latticeOut = new Lattice(x, y, z);
	}

	private void transformToP1() { // atomsIn -> atomsP1
		Vector aa = new Vector(100, 100);
		for (int i = 0; i < this.atomsIn.length; i++) {
			Vector3d v = new Vector3d(this.atomsIn[i].x, this.atomsIn[i].y, this.atomsIn[i].z);
			Vector3d[] vv = this.sgIn.getSymPos(v);
			for (int j = 0, k = 0; j < vv.length; j++) {
				if (vv[j] == null)
					continue;
				AtomSite a = new AtomSite(this.atomsIn[i]);
				// vv[j]=SpaceGroup.modCell(vv[j]);
				a.x = vv[j].x;
				a.y = vv[j].y;
				a.z = vv[j].z;
				if (vv.length > 1)
					a.label = a.label + "_s" + (k++);
				a.multiplicity = 1;
				aa.add(a);
			}
		}
		this.atomsP1 = (AtomSite[]) aa.toArray(new AtomSite[0]);
	}

	private void calculateAtomsOut() { // atomsP1 -> atomsOut
		Vector aa = new Vector(100, 100);
		Vector v = this.calculateTranslations();
		for (int i = 0; i < this.atomsP1.length; i++) {
			AtomSite a = new AtomSite(this.atomsP1[i]);
			Vector3d u = new Vector3d(this.atomsP1[i].x, this.atomsP1[i].y, this.atomsP1[i].z);
			this.Q.transform(u);
			u.add(this.q);
		...
		this.atomsOut = (AtomSite[]) aa.toArray(new AtomSite[0]);
	}

	private Vector calculateTranslations() {
		System.out.println("det(P)=" + this.P.determinant());

		Vector v = new Vector();
		int n = (int) Math.abs(Math.round(this.P.determinant()));
		if (n == 1)
			return v;

		Point3d p100 = new Point3d(1, 0, 0);
		this.Q.transform(p100);
		Point3d p010 = new Point3d(0, 1, 0);
		this.Q.transform(p010);
		Point3d p001 = new Point3d(0, 0, 1);
		this.Q.transform(p001);
				...
				for (int j = 0; j <= l; j++)
					for (int k = 0; k <= l; k++) {
						...
						if (!isAlreadyInOrZero(t, v)) {
							...
							System.out.println((i > 0 ? (i > 1 ? (i + "x") : "x") : "")
									+ (j > 0 ? (i > 0 ? "+" : "") + (j > 1 ? (j + "y") : "y") : "")
									+ (k > 0 ? (i > 0 || j > 0 ? "+" : "") + (k > 1 ? (k + "z") : "z") : "") + "  "
									+ (round(t)));
							n--;
							if (n == 1) {
								System.out.println();
	...
	public static Vector3d round(Vector3d p) {
		return new Vector3d(Math.round(100000 * p.x) / 100000d, Math.round(100000 * p.y) / 100000d,
				Math.round(100000 * p.z) / 100000d);
	}
}

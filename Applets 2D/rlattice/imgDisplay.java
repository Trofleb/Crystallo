...
class imgDisplay extends Canvas {
   ...
   public imgDisplay(String name, Pane p, URL u, boolean q, int offset, JApplet a) {
    this.offset = offset; 
    this.panel = p;
   ...
   public boolean action(Event evt, Object arg) {
     if ("Go Back".equals(arg) || "Done".equals(arg)) {
      ...
      return true;
       ...
       try {
         apl.getAppletContext().showDocument(new URL(url,"index.html"));
       } catch (Exception e) {
       	System.err.println("Unable to go to web page.");
       }
       return true;
...
/*
       ...
       img = tool.getImage(new URL(url, imageName));
...
*/
       ...
       u = new URL(url, dataName);
       try {
         URLConnection c = u.openConnection();
         StreamTokenizer st = new StreamTokenizer(c.getInputStream());
         if (st != null) {
...
//       System.out.println("offscreensize:  " + offScreenSize);
   ...
   public void paint(Graphics g) {
   	...
   	//System.out.println(state+" "+previous+" "+next);
   	
   	if (img == null)
       init();
     setUpCache();
     Dimension d = size();
//     System.out.println("drawing state:  " + state);
     if (state == STARTING) {
...
//     System.out.println("newxoff: " + newxoff + ", newyoff: " + newyoff);
   ...
   public void showDescription(String s) {
     if (desc != s) {
       desc = s;
       try {
//         apl.getAppletContext().showDocument(new URL(url,s),"descFrame");
         //TODO: FIX THIS PROPERLY (what is it supposed to do? a pop-up?
